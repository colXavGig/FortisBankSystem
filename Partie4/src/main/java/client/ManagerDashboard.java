...
public class ManagerDashboard {
	...
	private void checkAllAccountBalance() {
		...
		try {
			list.addAll(CheckingAccountCollection.getInstance().getData());
			list.addAll(SavingAccountCollection.getInstance().getData());
			list.addAll(CreditAccountCollection.getInstance().getData());
			list.addAll(CurrencyAccountCollection.getInstance().getData());
		} catch (SQLException e) {
			CLI.println("Could not load accounts");
			CLI.promptEnterToClear();
			return;
		}
		if (list.isEmpty()) {
			CLI.println("No accounts found");
			CLI.promptEnterToClear();
			return;
		}
